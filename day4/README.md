# day4: Multi-session Web UI

This directory is **day4** of the `pi-agent-101` project — the first stop of Basic Agent (multi-session, browser version).
See the repo root [README.md](../README.md) for the full plan; the previous entry is [day3](../day3/README.md).
...
day4 has exactly one goal: **turn on multi-session**. Each session is an independent `AgentSession` instance with its own history, its own SSE subscriber set, and its own `isProcessing` mutex. Switching = frontend closes the EventSource + backend swaps sessionId, with no history loss and no collisions.
...
`session.subscribe()` in day4 still translates only three events:
...
This is the minimum persistence for client state (a few lines of localStorage) and does not involve the SDK's SessionManager persistence — that's day9's content.
...
Explained in the order they appear in day4's code:

| Term | Explanation |
| --- | --- |
| **Multi-session** | Multiple independent `AgentSession` instances coexisting in one process, each maintaining its own messages array and LLM loop. **Do not** confuse this with `AgentSessionRuntime` — that's a "single current session + switch" UX model (used in day5). |
| **Entry** | A composite type day4 defines itself, bundling `AgentSession` + SSE subscriber Set + mutex + unsubscribe handle. The map's value is Entry; all CRUD goes through it. |
...
| **localStorage** | The browser's native persistent KV (day4 uses it to store currentId). After a process restart / browser reopen, the session can "revive" — but only the current one, not the entire sidebar list (that needs SessionManager persistence). |
...
- **Persistence**: Swap `SessionManager.inMemory()` for `SessionManager.create(process.cwd())`; after a process restart `createAgentSession` will automatically restore session history — but day4's Map is still cleared, so the sidebar list will be empty (day9 solves this).
- **History replay**: When switching to an old session, the frontend currently does `clearLog` then waits for new events. Reading `session.state.messages` and rendering history at once gives a more complete experience. day9 will do this.
...
Enter **day5: multi-session management (Electron version)**. On top of day4's Electron + IPC+preload app, plug in the SDK's `createAgentSessionRuntime` + `runtime.newSession` / `switchSession`, and hand off the hand-written `Map<id, Entry>` to the runtime.
