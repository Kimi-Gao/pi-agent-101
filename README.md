...
| [**day3**](./day3/) | ⬜ | **Desktop Agent 起步**（单会话形态） — 第一次接触 Electron：单会话（跟 day2 对齐）+ **transport 用 native IPC**：HTTP/SSE 全砍，主进程直接持有 pi session，渲染进程通过 `contextBridge` 暴露的 `pi.prompt()` / `pi.on('text_delta', ...)` 跟主进程对话 | `electron` + `ipcMain.handle` / `webContents.send` / `contextBridge.exposeInMainWorld` + `createAgentSession` |
...
| day7 | ⬜ | **Web Search 工具** — 第一个真正调用外部 API 的自定义工具；agent 不再只靠训练数据，能查实时信息 | `defineTool` 注册 `web_search(query)`；调搜索 API 返回结果；前端把搜索结果渲染成工具卡片 |
| day8 | ⬜ | 思考过程可视化 | `thinking_delta` 事件 |
